...
class Learning():
    def __init__(self,
        ...
        if self.summary_file.is_file():
            self.best_score = pd.read_csv(self.summary_file).best_metric.min()
    ...
    def valid_epoch(self, model, loader, local_metric_fn):
        ...
        for idx, batch in enumerate(tqdm_loader):
            with torch.no_grad():
                batch_loss = self.batch_valid(model, batch)
                current_loss_mean = (current_loss_mean * idx + batch_loss) / (idx + 1)

                tqdm_loader.set_description(f'val_loss: {current_loss_mean:.5f}')

        if self.distrib_config['DISTRIBUTED']:
            current_loss_mean = reduce_tensor(current_loss_mean, self.distrib_config['WORLD_SIZE'])
        # empty_cuda_cache()
